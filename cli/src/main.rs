...
use llamadb::connection::Connection;
        ...
        match val {
            ...
            Some(input) => {
                if input == "testdata" {
                    ...
                    match load_testdata(&mut sink, &mut conn) {
                ...
                while let Some(i) = lexer.tokens.iter().position(|token| token == &llamadb::sqlsyntax::lexer::Token::Semicolon) {
                    match execute(&mut out, &mut conn, &lexer.tokens[0..i+1]) {
                        Ok(()) => (),
                        Err(message) => println!("{}", message)
                    };
...
fn execute(out: &mut Write, conn: &mut Connection, tokens: &[llamadb::sqlsyntax::lexer::Token])
-> Result<(), String>
{
    ...
    execute_statement(out, conn, statement)
}

fn execute_statement(out: &mut Write, conn: &mut Connection, statement: llamadb::sqlsyntax::ast::Statement)
-> Result<(), String>
{
    use llamadb::ExecuteStatementResponse;

    let now = Instant::now();
    let execute_result = Some(conn.execute_statement(statement));
...
fn load_testdata(out: &mut Write, conn: &mut Connection) -> Result<(), String> {
    let test_data = include_str!("testdata.sql");

    let statements = llamadb::sqlsyntax::parse_statements(test_data);

    for statement in statements {
        try!(execute_statement(out, conn, statement));
    }
